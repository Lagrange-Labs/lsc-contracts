...
async function deployPoseidon() {
  ...
  const signerNode = new ethers.Wallet(
  ...
  for (it = 0; it < hashNums.length; it++) {
    ...
    cf = new ethers.ContractFactory(poseidonABI, poseidonCode, signerNode);
    cd = await cf.deploy();
    if (i == 2) {
  ...
  fs.writeFile(
    path.join(__dirname, '../script/output/deployed_poseidon.json'),
    jsonAddrs,
    (err) => {
      if (err) {
