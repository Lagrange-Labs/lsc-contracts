...
async function main() {
    ...
    let triageABIraw = fs.readFileSync('out/SlashingSingleVerifierTriage.sol/SlashingSingleVerifierTriage.json');
    ...
    console.log("slashing_single verifier contract deployed to address",verifier.address);
    
    // associate
    tx = await triage.setRoute(1, verifier.address);
    console.log("verifier contract associated with triage contract at",triage.address);
    await tx.wait();

    // deploy - aggregate
    
    triageABIraw = fs.readFileSync('out/SlashingAggregate16VerifierTriage.sol/SlashingAggregate16VerifierTriage.json');
	...
	// deploy
        ...
        verifier = await factory.deploy();
	tx = await verifier.deployed();
	console.log("slashing_aggregate verifier contract for size",size,"deployed to address",verifier.address);
