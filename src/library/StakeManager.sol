    ...
    IVoteWeigher
{
    ...
    function withdraw(address token, uint256 amount) external {
        ...
        require(
            lastStakeUpdateBlock[msg.sender] < block.number,
            "Stake is locked"
        );
        require(
            operatorStakes[token][msg.sender] >= amount,
    ...
    function setTokenMultiplier(
        ...
        tokenMultipliers.push(
            TokenMultiplier({token: token, multiplier: multiplier})
        );
    }

    function setQuorumIndexes(
        uint8 quorumNumber,
    ...
    function weightOfOperator(
        ...
        for (
            uint256 i = 0;
            i < quorumIndexes[uint8(quorumNumber)].length;
            i++
        ) {
            uint8 index = quorumIndexes[uint8(quorumNumber)][i];
            weight +=
                (operatorStakes[tokenMultipliers[index].token][operator] *
                    tokenMultipliers[index].multiplier) /
                _WEIGHTING_DIVISOR;
        }
    ...
    function freezeOperator(
        address operator
    ) external override onlyServiceManager {
