...
contract OptimismVerifier {
	...
	bytes32[4] outputProof
        ...
        bytes memory call = abi.encodeWithSignature("getL2OutputAfter(uint256)", comparisonNumber);
        (bool success, OutputProposal memory result) = L2OutputOracle.call(data);
	...
	require(outputRootProof == comparisonProof, "Output Root Proofs do not match");
