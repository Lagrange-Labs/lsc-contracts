...
async function testDefaultFreeze(lagrangeService) {
  ...
  frozenStatus = await lagrangeService.getFrozenStatus(
    '0x6E654b122377EA7f592bf3FD5bcdE9e8c1B1cEb9',
  );
  freezeException = false;
  try {
    await lagrangeService.freezeOperator(
      '0x6E654b122377EA7f592bf3FD5bcdE9e8c1B1cEb9',
    );
  } catch (error) {
...
async function getLagrangeCommittee(lagrangeService) {
  ...
  const lgrcABI = await fs.readFileSync(
    path.join(__dirname, '../out/LagrangeCommittee.sol/LagrangeCommittee.json'),
    'utf-8',
  );
...
async function getWETH9() {
  const currentProvider = await getProvider();
  const signerNode = await getSigner();
  deployFile = await fs.readFileSync(
    path.join(__dirname, '../broadcast/DeployWETH9.s.sol/1337/run-latest.json'),
  );
  ...
  const w9ABI = await fs.readFileSync(
    path.join(__dirname, '../out/WETH9.sol/WETH9.json'),
    'utf-8',
  );
...
async function getLagrangeService(redeploy) {
  ...
  if (redeploy) {
    ...
    const deploy = await exec.execSync(
      'npx hardhat deploy --network privnet',
      options,
    );
    ...
    const nsABI = await fs.readFileSync(
      path.join(__dirname, '../out/LagrangeService.sol/LagrangeService.json'),
      'utf-8',
    );
    ...
    const lagrangeService = new ethers.Contract(
      nsAddr,
      sanitizedABI,
      signerNode,
    );
    ...
    deployFile = await fs.readFileSync(
      path.join(__dirname, '../broadcast/Deploy.s.sol/1337/run-latest.json'),
    );
    deployJson = await JSON.parse(deployFile);
    deployTxns = deployJson.transactions;
    nsAddr = null;
...
async function testAddStakeIdent(nodeStaking) {
  ...
  try {
    await nodeStaking.addStakeIdent(
      1,
      pub.serialize(),
      publicKey,
      dataRoot,
...
async function testVerifyStateRoot(nodeStaking) {
  ...
  blockHash = await nodeStaking.calculateBlockHash(
    '',
  );
  assert.equal(
    blockHash,
  ...
  // invalid block hash
  try {
    res = await nodeStaking.verifyStateRoot(
...
async function getStrategyManager(lagrangeService) {
  ...
  const smABI = await fs.readFileSync(
    path.join(__dirname, '../out/IStrategyManager.sol/IStrategyManager.json'),
    'utf-8',
  );
...
async function getWETHStrategy(lagrangeService) {
  ...
  const wsABI = await fs.readFileSync(
    path.join(__dirname, '../out/IStrategy.sol/IStrategy.json'),
    'utf-8',
  );
...
//            console.log(await smContract.getDeposits("0xb2AaA94B0dbc3Af219B5abD7a141d0F66d55fB82"));
  ...
  describe('Lagrange Committee Smoke Tests', async function () {
    ...
    it('Poseidon Profile Tests', async function () {
      ...
      pubKey =
        '0xa936a5541b90dc5d48f7c9bea8979a8af4fc9ce7bc104932684084f6a7da3d5ace59afbdf0be4f3678171b2b3e70e0ce';
      ...
      p6 = await lgrc.hash4Elements(bytes6[0], bytes6[1], bytes6[2], bytes6[3]);

      console.log('hash4:' + p6.toString());

      p5 = await lgrc.hash3Elements(bytes5[0], bytes5[1], bytes5[2]);
