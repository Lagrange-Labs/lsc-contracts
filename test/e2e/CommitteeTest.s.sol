...
contract CommitteeTest is Script, CommitteeTreeTest {
    ...
    function setUp() public override {
        ...
        lagrangeCommittee = LagrangeCommittee(stdJson.readAddress(deployLGRData, ".addresses.lagrangeCommittee"));
        lagrangeService = LagrangeService(stdJson.readAddress(deployLGRData, ".addresses.lagrangeService"));
    ...
    function run() public {
        ...
        uint256[][_OPERATOR_COUNT] memory blsPrivateKeysArray;
        ...
        for (uint256 i; i < _OPERATOR_COUNT; i++) {
            _registerOperator(operators[i], privateKeys[i], amounts[i], blsPrivateKeysArray[i], chainIDArb);
        }

        ILagrangeCommittee.CommitteeData memory cur;
        {
            (uint256 startBlock,, uint256 _genesisBlock, uint256 duration, uint256 freezeDuration,,,) =
                lagrangeCommittee.committeeParams(chainIDArb);
        ...
        for (uint256 i; i < _OPERATOR_COUNT; i++) {
            ...
            uint96[] memory individualVotingPowers =
                lagrangeCommittee.getBlsPubKeyVotingPowers(operators[i], chainIDArb);
...

        {
            ...
            IBLSKeyChecker.BLSKeyWithProof memory blsKeyWithProof =
                calcProofForBLSKeys(operators[0], additionalBlsPrivateKeys, bytes32("salt2"), block.timestamp + 60);
