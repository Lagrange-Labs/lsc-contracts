...
contract.getEpochNumber(arbChainID, 10007664).then((epoch) => {
  console.log('Arb epoch: ', epoch);
});

contract.getCommittee(arbChainID, 10007664).then((current) => {
  console.log('Arb Current committee: ', current[0]);
  console.log('Arb Next committee: ', current[1]);
});

contract.getEpochNumber(optChainID, 10007664).then((epoch) => {
